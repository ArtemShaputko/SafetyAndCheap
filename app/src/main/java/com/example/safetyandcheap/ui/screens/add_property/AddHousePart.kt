...
import com.example.safetyandcheap.ui.util.SingleSelectTagsRow
...
fun AddHousePart(
    ...
    Column(
        ...
        FilterText("Input amount of bedrooms")
        Spacer(Modifier.height(10.dp))
        ContrastNumberSwitcher(
            onOptionSelected = { viewModel.updateBedroomsNum(it) },
            min = 1,
            max = 5,
            selectedOption = viewModel.bedroomsNum,
            modifier = Modifier.fillMaxWidth()
        )
        errorFields["bedroomsNum"]?.let { message ->
        ...
        OutlinedContrastTextField(
            modifier = Modifier.fillMaxWidth(),
            value = viewModel.siteArea,
            onValueChange = { viewModel.updateSiteArea(it) },
            label = "input site area",
            keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
            isError = errorFields["siteArea"] != null || networkError != null
        )
        errorFields["siteArea"]?.let { message ->
            ErrorText(message = message)
        }
        Spacer(Modifier.height(20.dp))
        ...
        SingleSelectTagsRow(
            ...
            onSelectedChange = { viewModel.updateEarthType(it) },
            selectedTag = viewModel.selectedEarthStatus
        )
        errorFields["selectedEarthType"]?.let { message ->
            ErrorText(message = message)
        }
        Spacer(Modifier.height(10.dp))
    }
